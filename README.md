...
| Авто-финиш | Каждые 5 минут проверяет окончание конкурса и публикует победителя в канал |
...
- Каждые 5 минут `_contest_watcher` проверяет `ends_at`, при истечении:
...
**`guild.yaml`** — функциональные настройки:

| Ключ | Тип | Описание |
|------|-----|----------|
| `activity_tracked_category_ids` | list[int] | Категории каналов для трекинга |
| `activity_excluded_channel_ids` | list[int] | Каналы внутри категорий, которые не считаются |
| `activity_results_channel_id` | int \| null | Канал для анонса победителей (фолбэк) |

> Право запускать/вести конкурс — RBAC-перм `activity.contest.manage` (в PostgreSQL),
> а не роль в конфиге.

**Тексты** — `activity-bot.yaml` (база `example_activity-bot.yaml`), секция `contest`:
публичный вывод (анонс, итоги) и баннер `contest.banner_path` (путь к ассету конкурса,
пусто = без баннера). Применить: `make reload`.
